...
export default function UpdateListing() {
  ...
  const handleImageSubmit = (e) => {
    ...
    Promise.all(promises)
      .then((urls) => {
        setListing({
          ...listing,
          imageUrls: listing.imageUrls.concat(urls),
        });
  ...
  const hangleDeleteImage = (d_url) => {
    setListing({
      ...listing,
      imageUrls: listing.imageUrls.filter((url) => url !== d_url),
    });
    console.log("🚀 ~ hangleDeleteImage ~ formData:", listing);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (listing.offer && listing.discount_price === 0) {
      postError("Discount price can't be empty");
    }
    // check if discount_price >= price
    if (listing.offer && listing.price <= listing.discount_price) {
    ...
    listing && (
      <div className="z-50 flex flex-col items-center bg-[#090831] max-w-[100vw] min-h-[100vh] text-[#f5f5f5] p-6 pt-[90px] font-embed">
        ...
        <h1 className="self-end text-[#f5f5f5] text-[1.3vw] m-4">
          {listing.imageUrls.length}/10 Image(Max.)
        </h1>
        {/* form */}
        <form
        ...
        >
          ...
          <div className="flex justify-between gap-2">
            <label
              htmlFor="description"
              className="text-[#f5f5f5] text-[1.7vw]"
            >
              Des.
            </label>
            <textarea
